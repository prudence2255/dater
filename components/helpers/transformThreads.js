class TransformThreads {
  ...
  getProfilePicture(extras) {
    const withProfilePictures = extras.filter((extra) => extra.profile_picture);
    return withProfilePictures?.find(
      (extra) => extra?.profile_picture?.client_id !== this.authUser?.id
    );
  }

  getNewMessageCount(extras) {
    const clientNewMessages = extras?.find(
      (extra) => extra?.user_id == this.authUser?.id
    );
