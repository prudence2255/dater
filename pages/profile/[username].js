...
function Profile() {
  ...
  const { user, authUser, profilePic } = Imports.useSelector(
  ...
  return (
    <Imports.Layout>
      <Head>
        <title>
          {user?.first_name} {user?.last_name} on dater.com
        </title>
        <meta
          property="og:title"
          content={`${user?.first_name} | ${user?.last_name} on dater.com`}
        />
        <meta
          name="og:description"
          content={`${user?.self_summary}, find ${user?.first_name} on dater.com`}
        />
        ...
        <div className="row no-gutters ">
          <div className="col-md-4 basic-info pb-5">
            <div className="profile-info-card">
              <div className="card img">
                ...
                <img
                  className="card-img-top img-fluid profile-info-pic"
                  src={profilePic?.medium ?? "/male-avatar.png"}
                  alt="profile picture"
                  width="100"
                  height="200"
                />
          ...
          <div className="col-md-8 mt-2 pb-5">
            <div className="profile-info">
              ...
              {user.id !== authUser.id && <VisitorActions user={user} />}
...
export const getServerSideProps = Imports.wrapper.getServerSideProps(
  async ({ store, req, query }) => {
    ...
    await store.dispatch(
      Imports.getUser({
        url: `/api/clients/${query.username}`,
    ...
    await store.dispatch(
      myFriendRequests({
        url: `/api/my-friend-requests`,
        cookie: cookie.get("token"),
      })
    );
    await store.dispatch(
      getAuthUserFriends({ url: `/api/friends`, cookie: cookie.get("token") })
    );
  }
);
